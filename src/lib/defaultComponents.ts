  ...
  return React.createElement('div', {
    ...
    React.createElement('div', {
      ...
      React.createElement('div', { style: { display: 'flex', flexDirection: 'row', alignItems: 'center', gap: '10px' } },
        ...
        React.createElement('div', { style: { display: 'flex', flexDirection: 'column' } },
          React.createElement('div', { style: { fontSize: '16px', fontWeight: '600', color: '#E8E8E8', lineHeight: '1.2' } }, contactName),
          React.createElement('div', { style: { fontSize: '11px', color: isOnline ? '#22C55E' : '#8A8A8A', marginTop: '2px' } }, isOnline ? 'online' : 'offline')
        )
      ),
      React.createElement('div', {
    ...
    React.createElement('div', {
      style: { flex: 1, overflowY: 'auto', padding: '8px 0 12px', display: 'flex', flexDirection: 'column', gap: '6px' }
    },
      (messages || []).length === 0
        ...
        : (messages || []).map(function(msg) {
            return React.createElement(MessageBubble, { key: msg.id, id: msg.id, contactId: msg.contactId, content: msg.content, timestamp: msg.timestamp, isSent: msg.isSent, isRead: msg.isRead });
          }),
    ...
    React.createElement('div', {
      ...
      React.createElement('input', {
        ...
        onKeyDown: function(e) {
          if (e.key === 'Enter' && inputText && inputText.trim() && sendMessage) {
    ...
    React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        isSent ? React.createElement('div', {
          style: { fontSize: '10px', color: isRead ? '#FFFFFF' : 'rgba(255,255,255,0.5)' }
        }, String.fromCharCode(10003) + String.fromCharCode(10003)) : null
      )
    )
  );
}`
